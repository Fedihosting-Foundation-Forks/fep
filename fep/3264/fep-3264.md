...
This document describes an implementation of basic project planning and work coordination based on [ActivityPub](https://www.w3.org/TR/activitypub/) protocol and [Valueflows](https://valueflo.ws/) vocabulary. It includes planning what people want to do, and (optionally) recording what is done.

## History

Extension of ActivityPub protocol with Valueflows vocabulary was initially proposed by Lynn Foster in [FEP-d767](https://codeberg.org/fediverse/fep/src/branch/main/fep/d767/fep-d767.md).  FEP-d767 will be withdrawn in favor of smaller more focused FEPs, as initiated by silverpill in [FEP-0837 Federated Marketplace](https://codeberg.org/fediverse/fep/src/branch/main/fep/0837/fep-0837.md).  This FEP continues the smaller Valueflows extensions.
...
- `action` (REQUIRED): the type of economic transaction. If the commitment is an input, the value of this property SHOULD be one of: `work`, `use`, `consume`, `cite`, `pickup`, `accept`.  If the commitment is an output, the value of this property SHOULD be one of: `produce`, `deliverService`, `dropoff`, `modify`.  See [the action definitions and behaviors](https://www.valueflo.ws/concepts/actions/) for more information.
  ...
  "planIncludes": [
    {
      ...
      "content": "Meet at the orchard, bring your own basket and a lunch, and be ready for some outdoor fall fun.",
      "hasInput": [
        {
          "type": "Commitment",
    ...
    },
    {
      ...
      "hasInput": [
        ...
        },
        {
          ...
          "receiver": "https://project.example/actors/gleaning-collective",
          "content": "Diego will drop his cider press off early in the morning, and pick it up later."
        },
        {
          "type": "Commitment",
...
- `inputOf` (REQUIRED): the process for which this commitment is an input.
...
- `inputOf` (REQUIRED): the process for which this economic event is an input.
- `context` (OPTIONAL): the plan the economic event is part of.
- `resourceConformsTo` (RECOMMENDED): the type of an economic resource (can be a skill or type of work for the `work` action). Could be any URI.
- `resourceInventoriedAs` (OPTIONAL): the actual resource that the economic event is about.  It is REQUIRED if the resource inventory should be updated through this economic event.
- `resourceQuantity` (OPTIONAL): the amount and unit of the non-effort-based economic resource.  Either `resourceQuantity` or `effortQuantity` is RECOMMENDED, and both can be included in case of `use`. This is an object with two properties:
  - `hasUnit` (REQUIRED): name of the unit, according to [Ontology of units of Measure](http://www.ontology-of-units-of-measure.org/) classification.
  - `hasNumericalValue` (REQUIRED): amount of the resource.
- `effortQuantity` (OPTIONAL): the amount and unit of the work or use effort-based action. This is an object with two properties:
...
- `finished` (OPTIONAL): set to true if this economic event completes the commitment.
...
```json
{
  "@context": [
    "https://www.w3.org/ns/activitystreams",
    {
      ...
      "action": "vf:action",
      "resourceConformsTo": "vf:resourceConformsTo",
      "resourceInventoriedAs": "vf:resourceInventoriedAs",
      "resourceQuantity": "vf:resourceQuantity",
      "effortQuantity": "vf:effortQuantity",
      "hasUnit": "om2:hasUnit",
      "hasNumericalValue": "om2:hasNumericalValue"
