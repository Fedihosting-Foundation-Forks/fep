...
Conforming servers MAY implement one of two profiles:

1. Synchronous Mode profile (where the server returns the exported account data directly 
   in the response to the `POST` request)
2. Asynchronous Mode profile (aka batch mode)

#### Synchronous Response Mode

A server implementing this mode MUST perform the export operation directly, and returns
the resulting artifact in the response to the `POST` request outlined above.

Example synchronous request and response:
