...
```json
{
	...
	"id": "https://example.com/some-create",
	"actor": "https://example.com/actors/2",
	"type": "Activity",
	"object": "https://example.com/some-created-object",
	"inReplyTo": {
		"id": "https://example.com/some-other-create",
		"actor": "https://example.com/actors/1",
		"type": "Create",
		"object": "https://example.com/some-other-created-object",
	...
	"result": {
		...
		"actor": "https://example.com/actors/1",
		"type": "Add",
		"object": "https://example.com/some-create",
...
Upon receiving an activity with a side effect of adding something to a special collection, ActivityPub servers SHOULD generate and deliver an `Add` activity representing this side-effect. The ActivityPub server MAY require manual action by a user. The resulting stamp activity SHOULD be delivered to the `actor` of the activity that caused the side-effect, and additionally SHOULD either be resolvable (to allow direct same-origin checking) or otherwise include a cryptographic proof. If a cryptographic proof is included, the proof SHOULD expire after some reasonable window, beyond which point a new proof should be issued. If the resulting stamp activity is resolvable, it MAY become unresolvable after some time; HTTP caching SHOULD be used to indicate a time-to-live for ActivityPub clients to re-check cached stamps.
