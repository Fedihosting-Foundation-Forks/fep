...
discussionTo: https://socialhub.activitypub.rocks/t/standards-portability-tf-would-id-s-that-arent-server-rooted-urls-simplify-some-of-the-harder-portability-problems/4281
---
...
1. Recognize client-side cryptographic signatures as proof of authorship (by
   implementing [FEP-8b32: Object Integrity Proofs][FEP-8b32]), in addition to
   the current practice of relying solely on the instance URL.

2. This enables Identity Hosting to be separated from the other instance concerns.
   Actor profiles can now be hosted separately from the instance
...
3. This (optionally) enables message transfer and Inbox/Outbox hosting to be
   outsourced to separate service providers (the Actor profile links to these in
   the usual manner).

4. Separate object and collection hosting (#4) can also be delegated to many
